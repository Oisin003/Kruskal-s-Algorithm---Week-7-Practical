...
public class KruskalsAlgorithm extends JFrame
    ...
    enum States {
        DEFAULT, ADD_VERTEX, REMOVE_VERTEX, ADD_EDGE_1, ADD_EDGE_2, REMOVE_EDGE, SET_EDGE_WEIGHT, COMPUTE_MST
    }
    ...
    JButton addVertexButton, removeVertexButton, addEdgeButton,
            removeEdgeButton, setEdgeWeightButton, computeMstButton, clearButton, enterButton, createFromNumbersButton;
    JTextField weight, numbersInput;
    ...
    public KruskalsAlgorithm() {
        ...
        buttonPanel1.setLayout(new BoxLayout(buttonPanel1,
                BoxLayout.X_AXIS));
        buttonPanel1.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.black),
        ...
        buttonPanel2.setLayout(new BoxLayout(buttonPanel2,
                BoxLayout.X_AXIS));
        buttonPanel2.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.black),
        ...
        addVertexButton.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.green),
        ...
        removeVertexButton.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.green),
        ...
        addEdgeButton.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.green),
        ...
        removeEdgeButton.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.green),
        ...
        setEdgeWeightButton.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.green),
        ...
        weight.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.green),
                weight.getBorder()));
        ...
        computeMstButton.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.blue),
        ...
        enterButton.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.green),
        ...
        clearButton.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.red),
                clearButton.getBorder()));

        // Create button to generate graph from numbers
        createFromNumbersButton = new JButton("Create from Numbers");
        createFromNumbersButton.setMinimumSize(buttonSize);
        createFromNumbersButton.setPreferredSize(buttonSize);
        createFromNumbersButton.setMaximumSize(buttonSize);
        createFromNumbersButton.setAlignmentX(Component.CENTER_ALIGNMENT);
        createFromNumbersButton.setActionCommand("createFromNumbers");
        createFromNumbersButton.addActionListener(this);
        createFromNumbersButton
                .setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.magenta),
                        createFromNumbersButton.getBorder()));

        // Create text field for numbers input
        numbersInput = new JTextField("Enter numbers: 10 20 30");
        numbersInput.setMinimumSize(new Dimension(200, 30));
        numbersInput.setPreferredSize(new Dimension(200, 30));
        numbersInput.setMaximumSize(new Dimension(200, 30));
        numbersInput.setAlignmentX(1.0f);
        numbersInput.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.magenta),
                numbersInput.getBorder()));

        // Create third button panel for numbers input
        buttonPanel3 = new JPanel();
        Dimension panelSize3 = new Dimension(900, 50);
        buttonPanel3.setMinimumSize(panelSize3);
        buttonPanel3.setPreferredSize(panelSize3);
        buttonPanel3.setMaximumSize(panelSize3);
        buttonPanel3.setLayout(new BoxLayout(buttonPanel3, BoxLayout.X_AXIS));
        buttonPanel3.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.black),
                buttonPanel3.getBorder()));
        ...
        buttonPanel3.add(new JLabel("Enter numbers (space separated): "));
        buttonPanel3.add(Box.createHorizontalGlue());
        buttonPanel3.add(numbersInput);
        buttonPanel3.add(Box.createHorizontalGlue());
        buttonPanel3.add(createFromNumbersButton);
        buttonPanel3.add(Box.createHorizontalGlue());

        contentPane.add((Component) this.canvas);
        contentPane.add(this.buttonPanel1);
        contentPane.add(this.buttonPanel2);
        contentPane.add(this.buttonPanel3);
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        mst.clear(); // when a new action is performed, the old MST may change, so we want to clear
                     // it
        canvas.repaint();
        ...
        if (buttonIdentifier.equals("addVertex")) {
            ...
            state = States.DEFAULT;
            canvas.repaint();
        } else if (buttonIdentifier.equals("createFromNumbers")) {
            // Create graph from numbers input
            createGraphFromNumbers();
        ...
        if (state != States.SET_EDGE_WEIGHT) {
            // when the user has not clicked on the set/change edge weight button, do not
    ...
    public int onVertex(Point point) {
        // if the point of the event occured on a vertex, return the index if that
        // vertex in vertices
        // if not, return -1
        int x = -1;
    ...
    public int onEdge(Point point) {
        // if the point of the event occured on an edge, return the index of that edge
        // in edges
        // if not, return -1
        int n = -1;
        ...
        if (clickedEdgeIndex > -1) {
            ...
            double distanceFromEdge = Line2D.ptSegDist(edge.v1.p.x, edge.v1.p.y, edge.v2.p.x, edge.v2.p.y, point.x,
                    point.y);
        ...
        for (int i = 0; i < edges.size(); ++i) {
            ...
            double distanceFromEdge2 = Line2D.ptSegDist(edge2.v1.p.x, edge2.v1.p.y, edge2.v2.p.x, edge2.v2.p.y, point.x,
                    point.y);
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        switch (state) {
            ...
            case ADD_EDGE_1: {
                clickedVertexIndex = onVertex(e.getPoint());
                if (clickedVertexIndex >= 0) {
                    // if the point where the user clicked is on a vertex, change to the state
                    // ADD_EDGE_2
            ...
            case ADD_EDGE_2: {
                ...
                if (vertexIndex != clickedVertexIndex && vertexIndex > -1) {
                    // if the point where the user clicked is on a vertex and a vertex that is
                    // different than the first, then create an edge
            ...
            case COMPUTE_MST:
                // Do nothing - MST is computed when button is clicked
                break;
        }
    }
    ...
    public void createGraphFromNumbers() {
        try {
            ...
            // Parse the input
            String input = numbersInput.getText().replaceAll("[^0-9\\s-]", ""); // Remove non-numeric chars except
                                                                                // spaces and minus
            String[] parts = input.trim().split("\\s+");

            if (parts.length < 2) {
                JOptionPane.showMessageDialog(this, "Please enter at least 2 numbers!");
                return;
            }
            ...
            int centerX = 450; // Center of canvas
            int centerY = 250;
            int radius = 150;

            for (int i = 0; i < numbers.length; i++) {
                double angle = 2 * Math.PI * i / numbers.length;
            ...
            // Create all possible edges with weights = absolute difference
            for (int i = 0; i < numbers.length; i++) {
                for (int j = i + 1; j < numbers.length; j++) {
                    ...
                    edge.weight = Math.abs(numbers[i] - numbers[j]);
            ...
            JOptionPane.showMessageDialog(this,
                    "Graph created with " + vertices.size() + " vertices and " + edges.size() +
                            " edges.\\nClick 'Compute MST' to see the minimum spanning tree!");

        } catch (NumberFormatException ex) {
            JOptionPane.showMessageDialog(this, "Please enter valid numbers separated by spaces!");
        }
    }
        ...
        switch (state) {
            ...
            case ADD_EDGE_2: {
                ...
                if (location > -1 && location != clickedVertexIndex) {
                    // if the location is on a vertex and it is not the same vertex as the one
                    // already selected for the new edge
